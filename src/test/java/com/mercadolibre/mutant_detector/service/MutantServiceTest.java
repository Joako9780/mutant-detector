...
class MutantServiceTest {
    ...
    void testAnalyzeNewMutant() {
        ...
        // Verificar
        assertTrue(result);
        verify(mutantDetector, times(1)).isMutant(dnaMutant); // Se llamó al detector
        verify(dnaRepository, times(1)).save(any(DnaRecord.class)); // Se guardó en BD
    }
    ...
    void testAnalyzeExistingDna() {
        // 1. Mock: YA existe en BD y es mutante
        DnaRecord existingRecord = new DnaRecord(dnaString, true);
        when(dnaRepository.findByDna(dnaString)).thenReturn(Optional.of(existingRecord));
        ...
        // Verificar
        assertTrue(result);
        verify(mutantDetector, never()).isMutant(any()); // ¡IMPORTANTE! No debe llamar al detector
        verify(dnaRepository, never()).save(any()); // No debe volver a guardar
    }
}